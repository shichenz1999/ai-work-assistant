...
def test_get_client_for_user_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    """Verifies the `get_client_for_user` factory contract."""
    # ARRANGE
    mock_client = Mock(spec=Client)
    mock_factory = Mock(return_value=mock_client)

    monkeypatch.setattr(mail_client_api, "get_client_for_user", mock_factory, raising=False)

    # ACT
    result = mail_client_api.get_client_for_user(user_id="user123")

    # ASSERT
    mock_factory.assert_called_once_with(user_id="user123")
