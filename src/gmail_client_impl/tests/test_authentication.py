...
class TestGmailClientAuthentication:
    ...
    @patch("gmail_client_impl.gmail_impl.build")
    def test_init_interactive_flow_returns_none_raises(self, mock_build: Any) -> None:
        """Test interactive flow failure raises a helpful error."""
        with patch.object(GmailClient, "_run_interactive_flow") as mock_interactive:
            mock_interactive.return_value = None

            with pytest.raises(RuntimeError, match=r"Interactive authentication failed\."):
                GmailClient(interactive=True)

        mock_build.assert_not_called()

    @patch("gmail_client_impl.gmail_impl.build")
    def test_init_invalid_credentials_raises_failure_message(self, mock_build: Any) -> None:
        """Test invalid credentials raise the standard failure message."""
        mock_creds = Mock(spec=Credentials)
        mock_creds.valid = False
        mock_creds.refresh_token = "refresh_token"

        with (
            patch.object(GmailClient, "_auth_from_env", return_value=mock_creds),
            pytest.raises(RuntimeError, match=GmailClient.FAILURE_TO_CRED),
        ):
            GmailClient()
...
class TestGmailClientHelperMethods:
    ...
    @patch("gmail_client_impl.gmail_impl.Path")
    @patch("gmail_client_impl.gmail_impl.Credentials")
    def test_auth_from_token_file_refresh_error_returns_none(
        self,
        mock_creds_class: Any,
        mock_path: Any,
    ) -> None:
        """Test token file refresh failure returns None."""
        ...
        assert client._auth_from_token_file("token.json") is None

    @patch("gmail_client_impl.gmail_impl.Path")
    @patch("gmail_client_impl.gmail_impl.Credentials")
    def test_auth_from_token_file_load_error_returns_none(
        self,
        mock_creds_class: Any,
        mock_path: Any,
    ) -> None:
        """Test token file load failure returns None."""
        mock_path.return_value.exists.return_value = True
        mock_creds_class.from_authorized_user_file.side_effect = RefreshError(  # type: ignore[no-untyped-call]
            "load failed"
        )
