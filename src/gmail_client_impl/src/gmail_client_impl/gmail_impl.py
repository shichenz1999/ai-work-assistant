...
class GmailClient(mail_client_api.Client):
    ...
    Authentication Flow:
        ...
        2. Try environment variables (GOOGLE_OAUTH_CLIENT_ID, GOOGLE_OAUTH_CLIENT_SECRET, GOOGLE_OAUTH_REFRESH_TOKEN)
    ...
    AUTH_DB_PATH: ClassVar[Path] = Path(
        os.environ.get("ORCHESTRATOR_AUTH_DB", "orchestrator_auth.db")
    )
    OAUTH_SCOPES: ClassVar[str] = os.environ.get("GOOGLE_OAUTH_SCOPES", "https://mail.google.com/")
...
def get_client_for_user_impl(user_id: str) -> mail_client_api.Client:
    """Return a configured :class:`GmailClient` instance for a user."""
    client_id = os.environ.get("GOOGLE_OAUTH_CLIENT_ID")
    client_secret = os.environ.get("GOOGLE_OAUTH_CLIENT_SECRET")
    token_uri = os.environ.get("GMAIL_TOKEN_URI", "https://oauth2.googleapis.com/token")
    if not client_id or not client_secret:
        raise RuntimeError("Gmail OAuth client is not configured.")  # noqa: TRY003, EM101

    refresh_token, scopes = _load_refresh_token(user_id)
    creds = Credentials(  # type: ignore[no-untyped-call]
        token=None,
        refresh_token=refresh_token,
        token_uri=token_uri,
...
def _load_refresh_token(user_id: str) -> tuple[str, list[str]]:
    if not user_id:
        raise RuntimeError("Missing user id for Gmail client.")  # noqa: TRY003, EM101
    if not GmailClient.AUTH_DB_PATH.exists():
        raise RuntimeError("No OAuth database found.")  # noqa: TRY003, EM101

    conn = sqlite3.connect(f"file:{GmailClient.AUTH_DB_PATH}?mode=ro", uri=True)
    conn.row_factory = sqlite3.Row
    try:
        row = conn.execute(
            "SELECT refresh_token, scopes FROM oauth_tokens WHERE user_id = ? AND provider = ?",
            (user_id, "google"),
        ).fetchone()
    except sqlite3.Error as exc:
        raise RuntimeError("Failed to read OAuth token.") from exc  # noqa: TRY003, EM101
    finally:
        conn.close()

    if not row or not row["refresh_token"]:
        raise RuntimeError("No OAuth token found.")  # noqa: TRY003, EM101
    ...
    if scopes_raw:
        try:
            ...
            scopes = []
    if not scopes:
        scopes = _parse_scopes(GmailClient.OAUTH_SCOPES)
    return str(row["refresh_token"]), [str(scope) for scope in scopes if scope]
...
def register() -> None:
    ...
    mail_client_api.get_client_for_user = get_client_for_user_impl
